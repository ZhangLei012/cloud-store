...
import (
	"cloud-store/service/dbproxy/conn"
	"database/sql"
	"log"
)
...
func OnFileUploadFinished(fileHash, fileName string, fileSize int64, fileAddr string) (res ExecResult) {
	...
	if rowAffected, err := result.RowsAffected(); nil == err {
		if rowAffected <= 0 {
			log.Printf("Error: inserting affects no rows, file hash %v had been inserted", fileHash)
		}
		res.Success = true
		return
	}
...
func GetFileMeta(fileHash string) (res ExecResult) {
	...
	statement, err := conn.DBConn().Prepare(
		"select file_hash, file_name, file_size, file_addr from tbl_file where file_hash = ? and status = 1 limit 1")
	if err != nil {
		log.Printf("Error: preparing statement, err:%v", err)
		return
	}
...
func UpdateFileLocation(fileHash string, fileAddr string) (res ExecResult) {
	res.Success = false
	statement, err := conn.DBConn().Prepare(
		"update tbl_file set `file_addr` = ? where `file_hash` = ? limit 1")
	if err != nil {
		log.Printf("Error: preparing statement, err:%v", err)
		res.Msg = err.Error()
		return
